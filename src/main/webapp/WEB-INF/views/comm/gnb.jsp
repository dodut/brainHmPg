    ...
    <div class="inner">
		...
		<nav>
			...
			<div id="pm2-lnb"> <!-- PC 메뉴 -->
				<ul class="dep1">
					<li <c:if test ="${ fn:contains(path, 'sub1_1') || fn:contains(path, 'sub1_2') || fn:contains(path, 'sub1_3') || fn:contains(path, 'sub1_4')}">class="on"</c:if> >
						<a href="/page/sub1_1">회사소개</a>
						<div>
							<ul class="dep2">
								<li <c:if test ="${fn:contains(path, 'sub1_1')}">class="on"</c:if> ><a href="/page/sub1_1">CEO인사말</a></li>
								<li <c:if test ="${fn:contains(path, 'sub1_2')}">class="on"</c:if> ><a href="/myboard/sub1_2">연혁&실적</a></li>
								<li <c:if test ="${fn:contains(path, 'sub1_3')}">class="on"</c:if> ><a href="/page/sub1_3">조직도</a></li>
								<li <c:if test ="${fn:contains(path, 'sub1_4')}">class="on"</c:if> ><a href="/page/sub1_4">찾아오시는 길</a></li>
							</ul>
						</div>
					</li>
					<li <c:if test ="${ fn:contains(path, 'sub2_1') || fn:contains(path, 'sub2_2') || fn:contains(path, 'sub2_3') || fn:contains(path, 'sub2_6') || fn:contains(path, 'sub2_4')}">class="on"</c:if> >
						<a href="/page/sub2_1">사업부문</a>
						<div>
							<ul class="dep2">
								<li <c:if test ="${fn:contains(path, 'sub2_1')}">class="on"</c:if> ><a href="/page/sub2_1">전자경매사업부</a></li>
								<li <c:if test ="${fn:contains(path, 'sub2_2')}">class="on"</c:if> ><a href="/page/sub2_2">POS사업부</a></li>
								<li <c:if test ="${fn:contains(path, 'sub2_3')}">class="on"</c:if> ><a href="/page/sub2_3">ICT사업부</a></li>
								<li <c:if test ="${fn:contains(path, 'sub2_6')}">class="on"</c:if> ><a href="/page/sub2_6">시너지유통사업팀</a></li>
								<li <c:if test ="${fn:contains(path, 'sub2_4')}">class="on"</c:if> ><a href="/page/sub2_4">SI사업부</a></li>
							</ul>
						</div>
					</li>
					...
					<li <c:if test ="${ fn:contains(path, 'sub4_1') || fn:contains(path, 'sub4_2')}">class="on"</c:if>>
						<a href="/page/sub4_1">인재채용</a>
						<div>
							<ul class="dep2">
								<li <c:if test ="${fn:contains(path, 'sub4_1')}">class="on"</c:if> ><a href="/page/sub4_1">채용절차</a></li>
								<li <c:if test ="${fn:contains(path, 'sub4_2')}">class="on"</c:if> ><a href="/page/sub4_2">복리후생</a></li>
							</ul>
						</div>
					</li>
				</ul>
...
<script>
	...
	function NavigationHorizon($this){
		var	$dep1 = $this.find('.dep1')
			,$lis1 = $dep1.children('li')
			,dep1right = $dep1.offset().left + $dep1.outerWidth();
		
		$lis1.each(function(){
			var	$dep2div = $(this).children('div');
			$(this).on('mouseenter',function(){
				var	dep2right = $(this).offset().left + $dep2div.outerWidth();
				$('#header .lnb_bg').show();
